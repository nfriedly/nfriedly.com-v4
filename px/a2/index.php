...
/*
   +-----------------+------------------------------------------------------------+
   |  Script         | A2 Based on PHProxy                                        |
   |  Author         | Abdullah Arif                                              |
   |  Last Modified  | 2010 - FEB                                   			  |
   |  Editor         | A2: [ http://a2freedom.wordpress.com ] 					  |
   |                 |     [ http://sourceforge.net/projects/a2freedom ]          |
   |                 |     [ http://a2freedom.blogspot.com ]                      |
   |                 |     [ http://a2freedom.4shared.com ]                       |
   |  Version        | 1.2														  |
   +-----------------+------------------------------------------------------------+
...
function url_parse($url, & $container)
{
    ...
    check_domain($temp['host'], $url);
    
		$GLOBALS['mp_domain'] = $temp['host'];
		$GLOBALS['mp_url'] = $url;
...
if ($_flags['a2_encode']) {
    ...
    {
        return rawurlencode(a2::encode($url));
    }
    function decode_url($url)
    {
        return str_replace(array('&amp;', '&#38;'), '&', a2::decode(rawurldecode($url)));
    }
}
elseif ($_flags['base64_encode'])
{
    function encode_url($url)
    {
        return rawurlencode(a2::_base64_encode($url));
    }
    function decode_url($url)
    {
        return str_replace(array('&amp;', '&#38;'), '&', a2::_base64_decode(rawurldecode($url)));
    }
}
elseif ($_flags['rotate13'])
{
    function encode_url($url)
    {
        return rawurlencode(a2::_str_rot13_encode($url));
    }
    function decode_url($url)
    {
        return str_replace(array('&amp;', '&#38;'), '&', a2::_str_rot13_decode(rawurldecode($url)));
    }
}
else
{
...
else
{
    ...
    if ($_flags['include_form'] && !isset($_GET['nf']))
    {
        ...
        // no robots
    $_response_body = str_replace("</head>", '<meta name="ROBOTS" content="NOINDEX" />' . "\r\n</head>", $_response_body);
}
